...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/') {
      return redirect(DEFAULT_ADMIN_PATH);
    }

    if (!env.ASSETS || typeof env.ASSETS.fetch !== 'function') {
      return new Response(
        'ASSETS binding manquant. Ajoutez une liaison Assets Cloudflare ou adaptez ce point pour servir votre admin.html.',
        { status: 500 }
      );
    }
